			...
			while(enumTopics.hasMoreElements()) {
				...
				for(int i=0;i<docs.length;i++){
					...
					for(int j=0;j<tfidf.length;j++) {
						int position = ((Integer)htWordOrder.get(wordsInTrainingCorpus[j])).intValue();
						double termFreq = ((Integer)currDoc.getTermFreq().get(wordsInTrainingCorpus[j])).doubleValue();
						double docFreq = ((Integer)htDocFreq.get(wordsInTrainingCorpus[j])).doubleValue();
						tfidf[j] = termFreq * Math.log(iDocumentCount / docFreq);
					}
	 ...
	 */
	 public void endElement(String namespaceURI, String localName, String qName) 
	 {
		...
		if(currElem.equals(DOCUMENT_TAG)) {
			...
			currDocument.setTermFreq(htDocFreq);
			iDocumentCount++;
			...
			htDocsByTopic.put(topic, documentSet);
		 ...
		 } else if(currElem.equals(DOCUMENT_TITLE_TAG) ||
					currElem.equals(DOCUMENT_CONTENT_TAG)) {
			 // Reading in data from either the document title or body.
			 ...
			 for(int i=0;i<words.length;i++)
			 {
				 ...
				 Integer wordFrequency = (Integer)htTermFreq.get(token);
				 if(wordFrequency == null) {
				 	/* THIS IS THE FIRST TIME WE SAW THIS WORD IN THE DOCUMENT */
					 wordFrequency = new Integer(1);
					...
					htDocFreq.put(token, documentFrequency);
					 ...
					 wordFrequency = new Integer(wordFrequency.intValue() + 1);
				 }
				 htTermFreq.put(token, wordFrequency);
			 }
		 }
