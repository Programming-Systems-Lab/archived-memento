	...
	implements NotificationListener
{
	...
	/**
	 * Construct a new ElvinEventService with the given session provider.
	 *
	 * @param sessProv SessionProvider to handle event sessions
	 */
	public ElvinEventService(SessionProvider sessProv)
	{
		super(sessProv);
	}

	/**
	 * When the event service is stopped, we close all open connections.
	 */
	public void stop()
	{
		super.stop();
	 ...
	 **/
	protected void publish(TopicUrl topic, Event event) throws EventException
	{
		Object connKey = makeConnectionKey(topic.getHostname(), topic.getPort());
		Connection conn = (Connection) connectionMap.get(connKey);
	 ...
	 **/
	public void addSubscription(TopicUrl topic) throws EventException
	{
		Object connKey = makeConnectionKey(topic.getHostname(), topic.getPort());
		Connection conn = (Connection) connectionMap.get(connKey);
	 ...
	 **/
	protected void removeSubscription(TopicUrl topic) throws EventException
	{
		Object connKey = makeConnectionKey(topic.getHostname(), topic.getPort());
		Connection conn = (Connection) connectionMap.get(connKey);
	 ...
	 **/
	private Subscription makeSubscription(TopicUrl topic)
	{
