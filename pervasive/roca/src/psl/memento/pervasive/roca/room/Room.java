...
import psl.memento.pervasive.roca.data.SizeData;
import psl.memento.pervasive.roca.data.CartesianCoord;
...
public class Room {
    ...
    public void setDoor(Door iDoor) {
      mDoor = iDoor;
    }
    ...
    public LinkedList[] getAllObjects() {
        LinkedList[] allObjects = new LinkedList[4];
        
        allObjects[0] = mStationary;
        allObjects[1] = mPervasive;
        allObjects[2] = mPhysical;
        allObjects[3] = mActive;
        
        return allObjects;
    }
    
    /* Indicates whether or not an object of size iSize beginning in
     * iCorner can fit in the room at this location. */
    public boolean canContain(SizeData iSize, CartesianCoord iCorner) {
        boolean fits = false;

        if ((iCorner.getX() >= 0) && ((iCorner.getX() + iSize.getWidth()) <= mSpanNSWalls) &&
            (iCorner.getY() >= 0) && ((iCorner.getY() + iSize.getLength()) <= mSpanWEWalls) &&
            (iCorner.getZ() >= 0) && ((iCorner.getZ() + iSize.getHeight()) <= mHeight)) {
                    fits = true;
        }
