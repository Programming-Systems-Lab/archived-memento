...
/**
 * Porter-Stemming algorithm for thre english language.
 */
public class StemmerEN implements Stemmer {
    ...
    /**
     * @param Word to stem
     * @return Stem of the word given as parameter
     */
    ...
    protected static String step5a (String str) {
        ...
        if ((stringMeasure(str.substring(0, str.length() - 1)) > 1) && str.endsWith("e"))
            return str.substring(0, str.length() -1);
        // (m = 1 and not *0) E ->
        else if ((stringMeasure(str.substring(0, str.length() - 1)) == 1) && (!endsWithCVC(str.substring(0, str.length() - 1))) && (str.endsWith("e")))
            return str.substring(0, str.length() - 1);
        else
            return str;
    }
