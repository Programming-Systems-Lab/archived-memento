...
public class ResourceFileManager {
	...
	private void readResourceFiles() throws IOException {
		...
		File resourceDir =
			new File(getBaseDir() + File.separator + kResourceDir);
		...
		if (contents == null) {
			System.out.println(
				"WARNING: ResourceFileManager found no resources in "
	...
	public ResourceFile getRF(int iResourceID) {
		return (ResourceFile) mResourceFiles.get(getHashKey(iResourceID));
	}

	public ResourceFile getRF(String name) {
		return (ResourceFile) mRFByName.get(name);
	}
	...
	public static ResourceFileManager getInstance(
		...
		if (iBaseDir == null) {
			...
			if (iUpdatePrefs)
				kPrefs.put(kKeyBaseDir, iBaseDir);
			return kSingleton;
		}
	}
